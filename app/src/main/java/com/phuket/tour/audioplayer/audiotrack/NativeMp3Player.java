...
public class NativeMp3Player {
	...
	private static final int BITS_PER_CHANNEL = 16;//量化格式
	private static final int CHANNEL_PER_FRAME = 2;//声道数
	...
	private int bitRate = 64000;//bps 数据比特率即每秒时间内的比特数。比特率 = sampleFormat(量化格式或位深度) * sampleRate(采样率) * channel(声道数)
	...
	private static int channelConfig = AudioFormat.CHANNEL_OUT_STEREO;//双声道输出
	...
	/**
	 * 调用解码器获取mp3文件的采样率和比特率，计算音频数据量，单位时间字节数、时长、解码缓存块大小。
	 * 重置播放状态isPlaying isStop
	 * 重置seek标识时间
	 */
	private boolean initMetaData(String path) {
		int[] metaArray = new int[] { 0, 0 };//采样率、比特率
		...
		totalCapacity = (new File(path)).length();//数据总量
		mp3CapacityPerSec = bitRate / BITS_PER_BYTE;//mp3单位时间内数据字节数
		if (mp3CapacityPerSec == 0) {
			return false;
		}
		duration = (int) (totalCapacity / mp3CapacityPerSec);//时长
		int byteCountPerSec = sampleRateInHz * CHANNEL_PER_FRAME * BITS_PER_CHANNEL / BITS_PER_BYTE;//单位时间内字节数
		DECODE_BUFFER_SIZE = (int) ((byteCountPerSec / 2) * 0.2);

		initPlayState();//重置播放状态
	...
	private void initAudioTrack() {
		int bufferSize = AudioTrack.getMinBufferSize(sampleRateInHz, channelConfig, audioFormat);//获取音频缓冲区大小
		...
		public void run() {
			...
			try {
				...
				while (!isStop) {
					...
					sample_count = decoder.readSamples(samples, extraSlientSampleSize);//读取采样

					Log.i(TAG, "run: extraSlientSampleSize[0] "+extraSlientSampleSize[0]+"- sample_count "+sample_count);
					...
					if (null != audioTrack && audioTrack.getState() != AudioTrack.STATE_UNINITIALIZED) {
						//?
						audioTrack.setStereoVolume(1, 1);//在音轨上设置指定的左右输出增益值。
						if (extraSlientSampleSize[0] > 0) {
							...
							audioTrack.write(playSamples, 0, totalSize);//阻塞方法，PCM采样写入缓冲区
						} else {
	...
	public int getCurrentTimeMills() {
		...
		try {
			if (null != audioTrack && audioTrack.getState() != AudioTrack.STATE_UNINITIALIZED) {
				...
				currentTimeMills = (int) ((float) curPos * 1000 / sampleRateInHz);//当前播放帧数 / 采样率 = 当前播放时间(s)
				currentTimeMills += seekBaseMillsTime;
				Log.i(TAG, "getCurrentTimeMills: PlaybackHeadPosition "+audioTrack.getPlaybackHeadPosition()+"- audioTrackBaseHeadPosition "+audioTrackBaseHeadPosition);
				Log.i(TAG, "getCurrentTimeMills: curPos "+curPos+"- sampleRateInHz "+sampleRateInHz+" - seekBaseMillsTime "+seekBaseMillsTime);
