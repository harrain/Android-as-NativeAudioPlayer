...
void AccompanyDecoderController::destroyDecoderThread() {
	...
	pthread_cond_signal(&mCondition);//额外发送一次signal指令，让解码线程有机会结束
...
int AccompanyDecoderController::readSamples(short* samples, int size,
	...
	if (packetPool->geDecoderAccompanyPacketQueueSize() < QUEUE_SIZE_MIN_THRESHOLD) {//判断队列音频帧数 是否小于 预设最小值
		int getLockCode = pthread_mutex_lock(&mLock);
		if (result != -1) {
			pthread_cond_signal(&mCondition); //发送signal指令让解码线程继续解码
		}
		pthread_mutex_unlock (&mLock);
	}
	return result;
